...
# Сценарий C: птица 12 000, остальные блоки −958,5 пропорционально → итого 100 000
APK_100_SCENARIO_C = [
    (1, "Птицеводческий комплекс «Нова-Агро» (слот кроликов)", 11041.5, 12000),
    (2, "Тепличный комплекс", 34500, 34128.3),
    (3, "Животноводческая ферма (КРС/МРС)", 27458.5, 27162.6),
    (4, "Рыбоводство (белуга)", 9000, 8903.0),
    (5, "Масложировой комбинат", 18000, 17806.1),
]


def add_apk_100_sheet(wb: openpyxl.Workbook) -> None:
    """Единый слайд: встройка 12 млрд птицы в контур APK 100 млрд (Приложение 3Б, сценарий C)."""
    if "APK-100" in wb.sheetnames:
        del wb["APK-100"]
    ws = wb.create_sheet("APK-100")
    ws["A1"] = "ВСТРОЙКА 12 млрд ПТИЦЫ В КОНТУР APK 100 млрд (Приложение 3Б)"
    ws["A1"].font = Font(bold=True, size=12)
    ws["A2"] = "Канон: сценарий C — итого 100 000 · apk-100bln-integration.md"
    ws["A3"] = "Земля птицы: APK 250 000 га, Запорожская обл. · слот 400 га · кадастр позже"

    headers = ["№", "Блок", "Baseline (кролики), млн ₽", "С птицей (C), млн ₽", "Δ, млн ₽", "Доля в 100 млрд"]
    for i, h in enumerate(headers, 1):
        ws.cell(5, i, h)
    style_header(ws, 5, len(headers))

    blocks = [(num, name, base, poultry, poultry - base) for num, name, base, poultry in APK_100_SCENARIO_C]
